...
export class AutomaticBookingDashboardPage extends Base {
  ...
  async assertPreviousPublishExternalListRunsTable(jobRun: string, locality: string, dateFrom: string, dateTo: string) {
    ...
    await this.page.locator(`div.vc-day:has(span.vc-day-content:has-text("${dateFrom}"))`).click();
    await this.page.locator(`div.vc-day:has(span.vc-day-content:has-text("${dateTo}"))`).click();
