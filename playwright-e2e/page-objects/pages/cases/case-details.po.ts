...
export class CaseDetailsPage extends Base {
  ...
  async checkInputtedCaseValues(
    ...
    //HMCTS case number
    await expect.soft(editNewCasePage.hmctsCaseNumberField)
    ...
    //Jurisdiction
    await expect.soft(editNewCasePage.jurisdictionField)
      .toHaveText("Jurisdiction " + jurisdictionType)
    //Service
    await expect.soft(editNewCasePage.serviceField)
      .toHaveText("Service " + serviceType)
    //CaseType
    await expect.soft(editNewCasePage.caseTypeField)
      .toHaveText("Case Type " + caseType)
    //region
    await expect.soft(editNewCasePage.regionField)
      .toHaveText("Region " + region)
    //cluster
    await expect.soft(editNewCasePage.clusterField)
      .toHaveText("Cluster " + cluster)
    //owning hearing
    await expect.soft(editNewCasePage.owningHearingField)
