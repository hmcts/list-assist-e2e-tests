...
export class EditNewCasePage extends Base {
  ...
  // Add new participant
  readonly caseParticipantsHeader = this.page.getByRole('heading', { name: 'Case Participants' });
  ...
  async createNewParticipant(
    ...
    await expect(createNewParticipant.getByRole('button', { name: 'Create New', exact: true })).toBeVisible();

    await createNewParticipant.getByRole('button', { name: 'Create New', exact: true }).click();

    // Fill in the participant details
    await expect(createNewParticipant.getByText('New Participant')).toBeVisible();
