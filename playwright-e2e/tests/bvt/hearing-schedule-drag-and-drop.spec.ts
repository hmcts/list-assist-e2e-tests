...
test.describe("Hearing Schedule - drag and drop @drag-and-drop", () => {
  test("Should move listing between sessions using drag and drop", async ({
    ...
    await test.step("Create released session for next working day from basket", async () => {
    ...
    await test.step("Create released placeholder session for today", async () => {
      await hearingSchedulePage.applyPrimaryDateFilter(
        dataUtils.generateDateInYyyyMmDdWithHypenSeparators(0),
        dataUtils.generateDateInYyyyMmDdWithHypenSeparators(0),
      );
      ...
      const row = table.filter(
        (r) =>
          r.roomName ===
          sessionBookingPage.CONSTANTS
            .CASE_LISTING_LOCATION_CARMARTHEN_CRTRM_01,
      )[0];
      ...
      await row[sessionBookingPage.CONSTANTS.CASE_LISTING_COLUMN_ONE]
        ...
        .click();
      await expect(
        hearingSchedulePage.schedulePopup.createSession,
      ).toBeVisible();
    ...
    await test.step("Filter for two days and verify released sessions in both columns", async () => {
      await hearingSchedulePage.sidebarComponent.openHearingSchedulePage();
      await hearingSchedulePage.applyPrimaryDateFilter(
        dataUtils.generateDateInYyyyMmDdWithHypenSeparators(adjustedOffset),
        dataUtils.generateDateInYyyyMmDdWithHypenSeparators(0),
    ...
    await test.step("Drag and drop listing from next day 10:00-11:00 to today slot 13:00-14:00 and verify move", async () => {
    ...
    await test.step("Drag and drop listing from today 13:00-14:00 to next day slot 10:00-11:00 and verify move", async () => {
      const caseName = process.env.CASE_NAME as string;
