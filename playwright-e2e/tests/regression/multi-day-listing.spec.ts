    ...
    test("Multi-day case listing @multi-day", async ({
        ...
        await test.step('Handle multi-day validation popup', async () => {
            ...
            await multiDayCartPage.handleListingValidationPopup(
              validationPopup,
              sessionBookingPage.CONSTANTS.CASE_LISTING_VALIDATION_POPUP_OVERRIDE_REASON
            );
        ...
        await test.step('Cancel 2 listings and assert multiday cart durations', async () => {
           ...
           // Wait for the section or table to be visible if needed
            ...
            await expect(hearingSchedulePage.cancelRescheduleReasonModel).toBeVisible();
            await hearingSchedulePage.cancelReasonDropDown.selectOption("CNCL");
            await expect(hearingSchedulePage.confirmationCanxAdvConfirm).toBeAttached();
            await expect(hearingSchedulePage.confirmationCanxAdvConfirm).toBeVisible();
            await expect(hearingSchedulePage.confirmationCanxConfirmYesButton).toBeVisible();
        ...
        await test.step('Assert multiday cart durations for partial listing', async ()=> {
            await multiDayCartPage.assertMultiDayCartDurations({
                ...
                remainingToAllocate: 'Remaining to allocate: session(s), total duration 00:00',
            });
        });
        ...
        await test.step('Assert multi-day cart durations after fulfilled', async () => {
            await multiDayCartPage.assertMultiDayCartDurations({
                ...
                listed: 'Listed:3 session(s), total duration 15:00',
                currentlySelected: 'Currently selected:0 session(s), total duration 00:00',
                remainingToAllocate: 'Remaining to allocate:1 session(s), total duration 02:00',
            });
        });
