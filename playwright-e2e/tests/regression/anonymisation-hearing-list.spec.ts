...
test.describe("Hearing List anonymisation @anonymisation @regression", () => {
    ...
    test("Civil - case name suppression", async ({
        ...
        const { caseNumber, caseName } = await test.step(
            "login and Create Civil Damages Small Claims case for case-name suppression",
            async () =>
        ...
        await test.step("verify external hearing list Welsh report shows suppressed case name and real party name", async () => {
            ...
            console.log("EXPECTED:", `${caseNumber} Hawliadau Bychain, ${caseNameSuppression}`);
    ...
    test("Civil case with parties & Alternative/Suppression applied to individual parties.", async ({
        ...
        // Create new Civil Damages Small Claims case
        const { caseNumber, caseName } = await test.step(
            "login and Create Civil Damages Small Claims case for party suppression",
            async () =>
                await createCivilDamagesSmallClaimsCase({
                    page,
                    loginPage,
...
// Reusable helpers
    ...
    async function createCivilDamagesSmallClaimsCase(
        pages: {
            page: Page;
            loginPage: LoginPage;
            homePage: HomePage;
