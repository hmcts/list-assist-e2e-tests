...
// Flag to track if cases have already been created
...
setup.describe('Global Setup', () => {
  ...
  setup(
    'Create new case',
    async ({ loginPage, addNewCasePage, config, page, homePage, hearingSchedulePage, dataUtils }) => {
      await page.goto(config.urls.baseUrl);
      await loginPage.login(config.users.testUser);
      ...
      //check if cases have already been created during that test run
      if (casesCreated) {
        return;
      }
